...
def test_at_median_user_lands_in_a_or_b() -> None:
    result = calculate_rank(
        commits=1000, prs=50, issues=25, reviews=2, stars=50, followers=10
    )
...
def test_high_performer_lands_in_a_plus_or_better() -> None:
    result = calculate_rank(
        commits=10_000, prs=500, issues=200, reviews=50, stars=5_000, followers=500
    )
...
def test_dineshs_current_stats_land_in_a_bucket() -> None:
    result = calculate_rank(
        commits=3800, prs=818, issues=36, reviews=10, stars=116, followers=20
    )
...
def test_increasing_commits_never_worsens_percentile() -> None:
    ...
    higher = calculate_rank(
        commits=10_000, prs=0, issues=0, reviews=0, stars=0, followers=0
    )
...
def test_increasing_stars_never_worsens_percentile() -> None:
    ...
    higher = calculate_rank(
        commits=0, prs=0, issues=0, reviews=0, stars=10_000, followers=0
    )
    assert higher.percentile <= base.percentile
